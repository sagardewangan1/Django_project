...
def customers(request):
    context = {}
    context['customers'] = Customer.objects.filter(user=request.user).values
...
def products(request):
    context = {}
    context['products'] = Product.objects.filter(user=request.user).values
...
#  this line .filter(user=request.user)= filters the Customer objects based on the user attribute, which is assumed to be a ForeignKey field linking to the Django User model. request.user represents the currently logged-in user making the request.
# def customersjson(request):
#     customers = list(Customer.objects.filter(user=request.user).values())
#     return JsonResponse(customers, safe=False)
